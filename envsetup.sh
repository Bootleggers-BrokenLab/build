...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/bootleggers/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/bootleggers/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function lunch()
{
    ...
    if (echo -n $1 | grep -q -e "^bootleggers_") ; then
      BOOTLEGGERS_BUILD=$(echo -n $product | sed -e 's/^bootleggers_//g')
    else
      BOOTLEGGERS_BUILD=
    fi
    export BOOTLEGGERS_BUILD
    BOOTLEGGERS_DEVICE=$BOOTLEGGERS_BUILD
    export BOOTLEGGERS_DEVICE

    local depsOnly=""
    if [[ $(find ./device -type d -name "$BOOTLEGGERS_DEVICE" -print -quit) != "" ]]; then
        depsOnly="true"
    fi

    cd $T > /dev/null
    vendor/bootleggers/build/tools/roomservice.py $product $depsOnly
    cd - > /dev/null
...
. $T/vendor/bootleggers/build/envsetup.sh
